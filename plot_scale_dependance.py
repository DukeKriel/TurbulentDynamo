...
def plotDependance(ax, labels_data, list_x, list_y, str_x, str_y, str_color, bool_fit_linear=False):
    ...
    ## label plot
    ax.set_xlabel(r"$ %s$"%(str_x), fontsize=22)
    ax.set_ylabel(r"$k_{%s}( %s)$"%(str_y, str_x), fontsize=22, color="black")
    ## add legend
...
ap.add_argument("-sub_folder",  type=str, default="", required=False, help="where spectras are stored in simulation folder")
ap.add_argument("-dat_labels",  type=str, default=None,   required=False, help="data labels", nargs="+")
ap.add_argument("-start_time",  type=int, default=0,      required=False, help="First file to process", nargs="+")
...
folders_sub   = args["sub_folder"]  # where spectras are stored in simulation folder
...
if len(start_time) < len(folders_data): start_time.extend( [0] * (len(folders_data) - len(start_time)) )
...
## folders where spectra data is stored
filepaths_data = []
...
for spectra_obj in spectra_objs:
    ...
    sim_time  = spectra_obj.vel_sim_times
...
## plot histograms
print("Plotting k_eta dependance...")
...
## save image
print("\t> Saving figure...")
fig_name = createFilePath([filepath_plot, pre_name]) + "_depend_k_eta.pdf"
...
fig, axs = plt.subplots(2, 2, figsize=(12,8), constrained_layout=True)
## plot histograms
print("Plotting k_nu dependance...")
...
## save image
print("\t> Saving figure...")
fig_name = createFilePath([filepath_plot, pre_name]) + "_depend_k_nu.pdf"
plt.savefig(fig_name)
plt.close()
print("\t> Figure saved: " + fig_name)
print(" ")
