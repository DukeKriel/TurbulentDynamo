...
ap.add_argument("-hide_progress_bar", type=str2bool, default=False, required=False, help="should the progress bar be hidden?", nargs="?", const=True)
ap.add_argument("-analyse", type=str2bool, default=True, required=False, help="analyse spectra and overwrite objects if necessary", nargs="?", const=True)
...
if bool_anlyse:
    ...
    for filepath_data, index in zip(filepaths_data, range(len(filepaths_data))):
        print("\tAnalysing spectra in: " + filepath_data)
        ...
        ## make sure that both velocity and magnetic fitting looked at the same amount of data
        tmp_vel_times = vel_args["vel_sim_time"]
        tmp_mag_times = mag_args["mag_sim_time"]
        if not(set(tmp_vel_times) == set(tmp_mag_times)):
            print("\t> There were {:d} velocity spectras and {:d} magnetic spectras analysed.".format(len(tmp_vel_times), len(tmp_mag_times)))
            print("\t> The times which weren't shared are:", list(set(tmp_vel_times) - set(tmp_mag_times)))
            raise Exception("The velocity and magnetic saved times aren't the same.")
        ## check which spectra files failed to be fitted
...
if bool_plot_scales:
    ...
    ax30.legend(fontsize=16, loc="upper right")
    ax31.legend(fontsize=16, loc="upper right")
    ## save image
...
if bool_plot_spectra:
    ...
    for spectra_obj, index in zip(spectra_objs, range(len(spectra_objs))):
        ...
        for sub_index in tqdm(range(len(sim_time)), disable=bool_hide_progress):
            ## plot spectras
            ax.plot(vel_k[sub_index], vel_power[sub_index], label=r"V-spectra", color="blue", ls="", marker=".", markersize=8)
            ax.plot(mag_k[sub_index], mag_power[sub_index], label=r"B-spectra", color="red", ls="", marker=".", markersize=8)
            ## plot measured scales
            if (sub_index_start <= sub_index) and (sub_index <= sub_index_end):
                if len(fit_vel_k) > 0:
                    ## plot fitted kinetic spectra
                    ax.plot(fit_vel_k[sub_index], fit_vel_power[sub_index], label=r"fitted V-spectra", color="blue", linestyle="--", dashes=(5, 2.5), linewidth=2)
                    ## plot scales
                    ax.axvline(x=k_nu[sub_index],    ls="--", color="blue",   label=r"$k_{\nu}$")
                    ax.axvline(x=k_nu_p[sub_index],  ls="--", color="green",  label=r"$k_{\nu}^\prime$")
                    ...
                    ## add model labels
                    ax.text(0.025, 0.025, r"$\mathcal{P}_{\rm vel}(k) = A\exp\left\{-\frac{k}{k_\nu^\prime}\right\} + \frac{\mathcal{P}_{\rm vel}(k_{\nu})}{1 + \left( \frac{k}{k_{\nu}} \right)^{\alpha}} \left( 1 - \exp\left\{-\frac{k}{k_\nu^\prime}\right\} \right)$", va="bottom", ha="left", transform=ax.transAxes, fontsize=12)
                if len(fit_mag_k) > 0:
                    ## plot fitted magnetic spectra
                    ax.plot(fit_mag_k[sub_index], fit_mag_power[sub_index], label=r"fitted B-spectra", color="red", linestyle="--", dashes=(5, 2.5), linewidth=2)
                    ## plot scales
                    ax.axvline(x=k_max[sub_index],   ls="--", color="black",  label=r"$k_{\rm max}$")
                    ax.axvline(x=k_eta[sub_index],   ls="--", color="red",    label=r"$k_{\eta}$")
                    ax.axvline(x=k_eta_p[sub_index], ls="--", color="orange", label=r"$k_{\eta}^\prime$")
                    ## show variance in variables
                    ...
                    ## add model labels
                    ax.text(0.025, 0.125, r"$\mathcal{P}_{\rm mag}(k) = A k^{\alpha_1}\exp\left\{-\frac{k}{k_\eta^\prime}\right\} + \frac{\mathcal{P}_{\rm mag}(k_{\eta})}{1 + \left( \frac{k}{k_{\eta}} \right)^{\alpha_2}} \left( 1 - \exp\left\{-\frac{k}{k_\eta^\prime}\right\} \right)$", va="bottom", ha="left", transform=ax.transAxes, fontsize=12)
            ## add time stamp
            ax.text(0.975, 0.975, r"$t/t_{\rm eddy} = $ "+"{:.2f}".format(sim_time[sub_index]), va="top", ha="right", transform=ax.transAxes, fontsize=16)
            ## add legend
            ax.legend(frameon=True, loc="upper left", facecolor="white", framealpha=0.5, fontsize=16)
            ## adjust figure axes
            ...
            ax.set_ylabel(r"$\mathcal{P}$", fontsize=14)
            ## save image
            temp_name = createFilePath([filepath_plot_spect, (pre_name + "_" + sim_label + "_spectra={0:04}".format(int(sub_index)) + ".png")])
